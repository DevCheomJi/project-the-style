...
import com.susu.projectthestyle.config.SecurityConfig;
...
@WebMvcTest(ArticleController.class) //테스트 대상 컨트롤러만 추가해주자.
class ArticleControllerTest {

    private final MockMvc mvc;

    public ArticleControllerTest(@Autowired MockMvc mvc) {//test 코드에서는 @autowired 생략 불가
        this.mvc = mvc;
    }
    ...
    public void givenNothing_whenRequestingArticleView_thenReturnsArticlesView() throws Exception {
        ...
        mvc.perform(get("/articles/1"))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.TEXT_HTML))
                .andExpect(view().name("articles/detail")) //view가 있어야 한다.
                .andExpect(model().attributeExists("article"))
                .andExpect(model().attributeExists("articleComments")) //댓글이 있어야 한다.
        ;
